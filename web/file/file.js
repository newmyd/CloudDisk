...
ajax("/file", "sort=" + sot + "&page=1&name=", false);
...
function ajax(url, str, opt) {
	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4 && xhr.status == 200) {
			if (opt) {
				ajax("/file",  "sort=" + sot + "&page=" + page + "&name=" + encodeURIComponent(sch), false);
...
function st(str, id, dir) {
	...
	ajax("/file",  "sort=" + sot + "&page=1&name=" + encodeURIComponent(sch), false);
}
...
function show(json) {
	...
	for (i = 1; i < len; ++i) {
		...
		file.innerHTML = file.innerHTML + "<tr><td><input id='file" + i + "' type='checkbox' class='checkbox' onclick='chck(" + i + ");' /></td><td>" + obj[i].name + "</td><td>" + sz.toFixed(3) + dw + "</td><td>" + obj[i].time + "</td></tr>";
	}
...
function cp() {
	ajax("/copy", "json=" + jsn(), true);
	return ;
}

function del() {
	ajax("/delete", "json=" + jsn(), true);
}

function rename() {
	if (document.getElementById("nameInput").value == "")
		return ;
	ajax("/rename", "name=" + encodeURIComponent(document.getElementById("nameInput").value) + "&json=" + jsn(), true);
...
function share() {
	if (document.getElementById("userInput").value == "")
		return ;
	ajax("/addshare", "user=" + encodeURIComponent(document.getElementById("userInput").value) + "&json=" + jsn(), true);

...
ajax("/viewshared", "sort=" + sot + "&page=1&name=", false);
...
function ajax(url, str, opt) {
	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4 && xhr.status == 200) {
			if (opt) {
				ajax("/viewshared",  "sort=" + sot + "&page=" + page + "&name=" + encodeURIComponent(sch), false);
...
function st(str, id, dir) {
	...
	ajax("/viewshared",  "sort=" + sot + "&page=1&name=" + encodeURIComponent(sch), false);
}
...
function show(json) {
	...
	for (i = 1; i < len; ++i) {
		...
		file.innerHTML = file.innerHTML + "<tr><td><input id='file" + i + "' type='checkbox' class='checkbox' onclick='chck(" + i + ");' /></td><td>" + obj[i].name + "</td><td>" + sz.toFixed(3) + dw + "</td><td>" + obj[i].user + "</td><td>" + obj[i].time + "</td></tr>";
	}
	for (i = 1; i < len; ++i) {

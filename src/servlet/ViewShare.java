...
public class ViewShare extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        ArrayList<Share> res = Share.getFileByUser(userId, name, sot, page * siz - siz, siz);

        JSONArray json = new JSONArray();
        json.add((Share.getSizeByUser(userId, name) + siz - 1) / siz);
        for (Share i : res) {
            JSONObject ob = new JSONObject();
            ob.put("id", i.getId());
            ob.put("name", i.getName());
            ob.put("src", User.getUserById(i.getSrc()).getUserName());
